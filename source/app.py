...
from ranger_bot import RangerReport
...
def ranger(user_id: str, channel: str):
    print("Hi")
    ranger_report = RangerReport(channel)
    # Create first message
    message = ranger_report.get_message_payload()
    print(message)
    sys.exit()
    # Post the onboarding message in Slack
    response = slack_web_client.chat_postMessage(**message)
...
def message(payload):
    ...
    if text and text.lower().startswith("ranger"):
        return ranger(user_id, channel_id)
